...
& \qquad \quad \eta \longleftarrow g(x,\eta)\\
& \qquad \quad x \longleftarrow x+\eta\nabla p\\
...
\item Función de tamaño del paso constante: $\eta_{k+1}=g(x_k,\eta_k)=\eta$
...
\item Función de tamaño del paso constante: $\eta_{k+1}=g(x_k,\eta_k)=\eta=5\times10^{-7}$
...
\item Función de tamaño del paso constante: $\eta_{k+1}=g(x_k,\eta_k)=\eta=10^{-7}$
...
\item Función de tamaño del paso constante: $\eta_{k+1}=g(x_k,\eta_k)=\eta=5\times 10^{-8}$
...
El último punto es crucial en el algoritmo implementado. Utilizar $\eta_{k+1}=g(x_k,\eta_k)=\eta$ puede ahorrar cálculos en la búsqueda de un $\eta$ que cumpla $f(x_{k+1}(\eta))<f(x_k)$. Por otro lado, para un $\eta$ fijo puede existir una iteración $q$ en donde $f(x_{q+1}(\eta))>f(x_q)$ que provoque una falla en el algoritmo. La forma correcta de abordar el problema es calcular en cada iteración una $\eta^*$ que cumpla con la condición de **Armijo-Goldstein**: $$f(x_{k+1})=f(x_k+\eta_k^*p_k) \leq f(x_k)+c\eta_k^*p_k^T\nabla f(x_k), c\in(0,1)$$
...
>>>>>>> 2fd995818616aa942e82accf8568fc35b4520130
